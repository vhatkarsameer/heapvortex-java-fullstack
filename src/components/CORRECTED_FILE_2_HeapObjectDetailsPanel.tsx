...
export const HeapObjectDetailsPanel: React.FC<HeapObjectDetailsPanelProps> = ({
  ...
  const generateReferenceChain = (address: string): string[] => {
    // Mock reference chain generation - in production, this comes from backend
    const chains = [
      ["GC Root", "ThreadGroup", "Thread", "ThreadLocal", address],
      ["Static Reference", "Class Loader", "Class", address],
      ["Heap Root", "Instance", address],
      ["Array Element", "Collection", address],
    ];
  ...
  const referenceChain = object.referenceChain || generateReferenceChain(object.address);
  const totalHeap = object.shallowHeap + object.retainedHeap;
        ...
        <div className="bg-gray-900 rounded-lg overflow-hidden">
          ...
          {expandedSections.has("overview") && (
            <div className="border-t border-gray-700 p-3 space-y-3">
              ...
              {/* Memory Address */}
              <div>
                <p className="text-xs text-gray-500 uppercase tracking-wide">Memory Address</p>
                <div className="flex items-center justify-between mt-1">
                  <p className="text-sm font-mono text-blue-400 break-all">
                    {object.address}
                  </p>
                  <button
                    onClick={() => copyToClipboard(object.address)}
                    className="p-1 hover:bg-gray-700 rounded transition-colors ml-2"
                    title="Copy address"
                  >
                    <Copy size={14} className="text-gray-400" />
              ...
              {/* Shallow Heap */}
              <div>
                <p className="text-xs text-gray-500 uppercase tracking-wide">Shallow Heap</p>
                <div className="mt-1 flex items-baseline gap-2">
                  <p className="text-lg font-bold text-blue-400">
                    {formatBytes(object.shallowHeap)}
                  </p>
                  <p className="text-xs text-gray-500">
                    ({object.shallowHeap.toLocaleString()} bytes)
                  </p>
                </div>
              </div>
              ...
              {/* Retained Heap */}
              <div>
                <p className="text-xs text-gray-500 uppercase tracking-wide">Retained Heap</p>
                <div className="mt-1 flex items-baseline gap-2">
                  <p className="text-lg font-bold text-green-400">
                    {formatBytes(object.retainedHeap)}
                  </p>
                  <p className="text-xs text-gray-500">
                    ({object.retainedHeap.toLocaleString()} bytes)
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="mt-2 w-full bg-gray-700 rounded-full h-2 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min((object.retainedHeap / 100000) * 100, 100)}%`,
                    }}
                  />
                </div>
                <p className="text-xs text-gray-400 mt-1">
                  {Math.min((object.retainedHeap / 100000) * 100, 100).toFixed(1)}% of max heap
                </p>
              </div>
            </div>
          )}
            ...
            <div className="border-t border-gray-700 p-3 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-gray-800 rounded p-2">
                  <p className="text-xs text-gray-500">Total Heap</p>
              ...
              <div className="bg-gray-800 rounded p-3 mt-3">
                <p className="text-xs text-gray-500 uppercase tracking-wide mb-2">Retention Ratio</p>
                  ...
                  <div className="flex-1">
                    <div className="w-full bg-gray-700 rounded-full h-3 overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${(object.shallowHeap / totalHeap) * 100}%`,
                        }}
                      />
                    </div>
                  </div>
                  <p className="text-xs text-gray-400 ml-2">
                    {((object.shallowHeap / totalHeap) * 100).toFixed(1)}%
                  </p>
                </div>
              </div>
            </div>
